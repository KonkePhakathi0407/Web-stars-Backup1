// MindCare Hub – Mood Tracker JavaScript
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSlider(val) {
    ...
    const thresholds = [{ max: 2, idx: 0 }, { max: 4, idx: 1 }, { max: 6, idx: 2 }, { max: 8, idx: 3 }, { max: 10, idx: 4 }];
    const btns = document.querySelectorAll(".emoji-btn");
    btns.forEach((b) => b.classList.remove("selected"));
    for (const t of thresholds) {
      if (val <= t.max) { btns[t.idx].classList.add("selected"); break; }
    }
  }
  ...
  // Build bar chart
  const moodData = [5,6,4,7,8,6,9,7,5,6,8,7,6,8,7,9,8,6,7,5,8,9,7,8,9,7,6,8];
